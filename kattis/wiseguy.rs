...
impl Table {
    fn new(rows: usize) -> Self {
        ...
        for n in 1..=rows {
            ...
            for k in 0..=n {
                last +=
                    (0..=k).map(|i| t.compute_probability(n, k, i)).sum::<f64>() / ((n + 1) as f64);
                t.0.push(last);
            }
        }
        t
    ...
    fn compute_probability(&self, n: usize, k: usize, i: usize) -> f64 {
        let j = n - i;
        self.get_cumul(i, k + 1) * self.get_cumul(j, k)
            + self.get_cumul(j, k + 1) * self.get_cumul(i, k)
            - 2. * self.get_cumul(i, k) * self.get_cumul(j, k)
    }

    fn expectation(&self, n: usize) -> f64 {
        let row_start = n * (n - 1) / 2;
        (0..n)
            .map(|k| (self.0[row_start + k + 1] - self.0[row_start + k]) * (k as f64))
            .sum()
    }
}
